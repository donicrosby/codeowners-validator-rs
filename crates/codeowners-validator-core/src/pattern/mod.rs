...
use globset::{Glob, GlobMatcher, GlobSetBuilder, GlobSet};
use std::path::Path;
...
fn normalize_pattern(pattern: &str) -> (String, bool, bool) {
    ...
    // Handle ** patterns
    // In CODEOWNERS, ** matches any path segment
    // globset handles this natively
...
mod tests {
    ...
    fn pattern_double_wildcard() {
        ...
        assert!(pattern.matches("a/test"));
        assert!(pattern.matches("a/b/test"));
        assert!(pattern.matches("a/b/c/test"));
    }
